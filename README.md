<p align="center">
  <img src="./src-tauri/icons/app.png" width="96" alt="帧野画布" />
</p>

<h1 align="center">帧野画布</h1>
...
<p align="center">
  帧野画布是面向 AI 影像创作者的桌面工作台——<br />
  把散落的提示词、参考图、生成结果和创作意图，<br />
  <strong>编织成一部真正可交付的作品</strong>，而不是散落各处、难以追溯的生成结果。
</p>

<p align="center">
  <strong>不绑定平台，不限定模型。</strong><br />
  你可以自由接入自己的 API 厂商，为文本、图片、视频和音频分别配置最适合的模型。
</p>

<p align="center">
  <a href="#-为什么是帧野画布">为什么是帧野画布</a> ·
  <a href="#-能力一览">能力一览</a> ·
  <a href="#-平台由你选择">平台配置</a> ·
  <a href="#-界面预览">界面预览</a> ·
  <a href="#-快速开始">快速开始</a> ·
  <a href="#-开发">开发</a>
</p>
...
![帧野画布创作界面](./docs/images/img_1.png)
...
| 被单一平台、固定模型和套餐绑定 | 自由配置 API 厂商、模型协议和参数，按任务选择最合适的平台 |
| AI 助手对你的项目一无所知 | 助手直接读取画布上下文，给你针对性建议 |

**帧野画布不是又一个 AI 生图工具。** 它是 AI 影像创作缺失的那一层——把提示词、素材、生成和后期串起来的工作台。
...
<table>
  ...
  <tr>
    <td width="50%">
      <h3>🔌 自由配置生成平台</h3>
      <p>不把创作流程锁在某一家平台。你可以添加自己的 API 厂商，配置接口地址、凭据、模型协议和参数，并为文本、图片、视频、音频分别选择默认模型。同一张画布可以衔接来自不同平台的能力。</p>
    </td>
    <td width="50%">
      <h3>📦 项目与素材管理</h3>
...
帧野画布管理的是你的创作工作流，而不是替某一家模型平台圈住用户。平台和模型都可以按项目需要自由组合：

- **自带 API**：添加你正在使用的 API 厂商，凭据保留在自己的桌面环境中。
- **自定义协议**：配置请求地址、传输方式、输入模式、参数和结果解析，不必等待应用内置每一家服务。
- **多平台协作**：文本构思、图片生成、视频生成和音频生成可以分别使用不同厂商、不同模型。
- **随时切换**：模型是节点配置，不是项目格式。保留画布、提示词和素材关系，只替换当前任务使用的生成能力。

你可以用熟悉的平台，也可以接入新的兼容服务；哪个模型更适合当前镜头，就用哪个。
...
<table>
  <tr>
    <td width="50%">
      <img src="./docs/images/img_2.png" alt="帧野画布项目库" />
      <p align="center"><strong>项目库</strong> — 创建、整理和打开本地项目</p>
    </td>
    <td width="50%">
      <img src="./docs/images/img.png" alt="帧野画布创作界面" />
      <p align="center"><strong>创作画布</strong> — 组织节点、素材关系与生成任务</p>
    </td>
  </tr>
  <tr>
    <td width="50%">
      <img src="./docs/images/img_3.png" alt="帧野画布 3D 导演台" />
      <p align="center"><strong>3D 导演台</strong> — 预演摄影机、角色与空间关系</p>
    </td>
    <td width="50%">
      <img src="./docs/images/img_4.png" alt="帧野画布视频编辑器" />
...
不接模型也能用来建项目和整理素材。帧野画布不要求绑定指定平台；要使用生成节点或 AI 助手时，接入你自己的服务即可：
