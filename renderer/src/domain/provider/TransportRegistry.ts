...
import {
  ...
  formatRemoteTaskError,
} from "@/utils/remoteTaskFormatting";
import type {
  ProviderTransport,
  ProviderTask,
  ProviderTaskState,
...
} from "@/utils/modelProtocol.mjs";
import { multipartResourceFieldName } from "@/utils/modelRequestBody.mjs";
...
class DeclarativeProviderTransport implements ProviderTransport {
  ...
  compileRequest(context: CompileContext): CompiledProviderRequest {
    ...
    const multipartResources =
      contract.inputFormat === "multipart"
        ? [
            ...this.compileMultipartResources(
              refs,
              contract.requestFields.multipartImage,
              "图片",
              "multipartImage",
              "input.png",
            ),
            ...this.compileMultipartResources(
              inputs.videos || [],
              contract.requestFields.multipartVideo,
              "视频",
              "multipartVideo",
              "input.mp4",
            ),
            ...this.compileMultipartResources(
              inputs.audios || [],
              contract.requestFields.multipartAudio,
              "音频",
              "multipartAudio",
              "input.wav",
            ),
        ...
        : undefined;
    if (contract.requestTemplate === undefined)
      throw new Error(
        `${contract.modelId}/${contract.modeId} 缺少声明式请求模板`,
      );
    return {
      ...
      protocolVariables: {
        ...
        messages: [
          ...
          {
            role: "user",
            content: `${context.prompt || ""}${context.upstreamContext ? `\n\n${context.upstreamContext}` : ""}`,
          },
        ],
        ...
        duration:
          ...
          context.duration ||
          contract.defaultDuration,
        aspectRatio:
          context.params?.aspectRatio || context.aspectRatio || context.ratio,
        ratio:
          context.params?.ratio ||
          context.ratio ||
          context.aspectRatio ||
          context.params?.aspectRatio,
      ...
      auth: contract.auth,
      responseEncoding:
        contract.resultBody?.encoding === "binary" ? "binary" : "json",
      contract,
  ...
  private compileMultipartResources(
    refs: ResourceRef[],
    fieldTemplate: string | undefined,
    resourceLabel: string,
    fieldKey: string,
    fallbackName: string,
  ): NonNullable<CompiledProviderRequest["inputResources"]> {
    if (!refs.length) return [];
    if (!fieldTemplate?.trim()) {
      throw new Error(
        `multipart 协议缺少 ${resourceLabel}文件字段，请在 requestFields 中配置 ${fieldKey}`,
      );
    }
    return refs.map((ref, index) => ({
      filePath: ref.filePath || undefined,
      url: ref.url || ref.remoteUrl || ref.previewUrl || undefined,
      fileName: ref.fileName || fallbackName,
      fieldName: multipartResourceFieldName(fieldTemplate, ref, index),
      mimeType: ref.mimeType,
    }));
  }

  async submit(request: CompiledProviderRequest): Promise<ProviderTask> {
    const imageContentFormat =
      request.contract?.requestFields?.imageContentFormat || "";
    const imageEntries = await collectProtocolResourceEntries(
      request.protocolImageRefs || [],
      {
    ...
    const videoUrls = await collectProtocolResourceValues(
      request.protocolVideoRefs || [],
      { fallbackMimeType: "video/mp4" },
    );
    const audioUrls = await collectProtocolResourceValues(
      request.protocolAudioRefs || [],
      { fallbackMimeType: "audio/wav" },
    );
    const inlineImage =
      imageContentFormat === "google-inline"
    ...
    if (
      ...
      throw new Error(
        `${request.contract?.modelId || "Google Veo"} 图生视频需要可读取的本地图片或 Base64 图片`,
      );
    }
    const imageRoleForSlot = (slot = "") => {
      const fields = request.contract?.requestFields || {};
      if (slot === "firstFrame")
        return fields.firstFrameImageContentRole || fields.imageContentRole;
      if (slot === "lastFrame")
        return fields.lastFrameImageContentRole || fields.imageContentRole;
    ...
    const klingContents = imageContentFormat.startsWith("kling-")
      ? protocolKlingContents({
          ...
          imageUrls,
          imageType:
            imageContentFormat === "kling-references"
              ? "refer_image"
              : "first_frame",
        })
      : undefined;
    const messageVariables = protocolMessageVariables(
      Array.isArray(request.protocolVariables?.messages)
        ? request.protocolVariables.messages
        : [],
    );
    const body = renderProtocolTemplate(request.protocolTemplate, {
      ...
      referenceImageUrls: imageEntries
        .filter(({ ref }) => !ref.inputSlot || ref.inputSlot === "reference")
        .map(({ value }) => value),
      firstFrameImageUrl: imageEntries.find(
        ({ ref }) => ref.inputSlot === "firstFrame",
      )?.value,
      lastFrameImageUrl: imageEntries.find(
        ({ ref }) => ref.inputSlot === "lastFrame",
      )?.value,
    ...
    if (request.endpointPath.includes("/images/")) {
      const data = await desktopApi.model.imageGeneration({
        ...
        __inputResources: request.inputResources || [],
    ...
    if (
      request.taskType === "videoGeneration" ||
      request.endpointPath.includes("/contents/generations/")
    ) {
  ...
  async poll(
    ...
    const encodedTaskId = task.remoteTaskId
      .split("/")
      .map(encodeURIComponent)
      .join("/");
    const ep = (contract.taskEndpoint?.path || "").replace(
      "{taskId}",
      encodedTaskId,
    );
    ...
    const rawStatus = pickScalar(
      firstProtocolValue(data, contract.statusPath || ""),
    );
    if (!rawStatus)
      throw new Error(
        `${contract.modelId}/${contract.modeId} 的轮询响应缺少状态路径 ${contract.statusPath}`,
      );
    const status =
      contract.pollStatusMap?.[rawStatus] || normalizeRemoteStatus(rawStatus);
  ...
  private submittedTask(
    ...
    const tid = contract.isAsync
      ? pickStr(firstProtocolValue(data, contract.taskIdPath || ""))
      : "";
    if (contract.isAsync && !tid)
      throw new Error(
        `${contract.modelId}/${contract.modeId} 的提交响应缺少任务 ID 路径 ${contract.taskIdPath}`,
      );
    const result = contract.isAsync
      ? { raw: data }
      : this.protocolResult(data, contract);
    return {
      remoteTaskId: tid,
  ...
  private protocolResult(
    ...
    if (
      !contract.resultUrlPath &&
      !contract.resultTextPath &&
      !contract.resultBase64Path &&
      !contract.resultHexPath &&
      !contract.resultBody
    ) {
  ...
  private completedProtocolResult(
    ...
    if (!contract.resultEndpoint?.path)
      return this.protocolResult(data, contract);
...
async function collectProtocolResourceValues(
  ...
  const values = await Promise.all(
    refs.map(async (ref) => {
      ...
      if (direct && !preferDataUrl && (remotelyUsable || !ref.filePath))
        return direct;
      if (ref.filePath) {
        try {
          ...
          /* fall through to a remote value when available */
        }
      }
      return direct;
    }),
  ...
  return {
    remoteTaskId: pickStr(
      data.taskId ||
        ...
        data.remoteTaskId ||
        data.serverTaskId ||
        fallback.remoteTaskId,
    ),
    status: normStatus(
      pickStr(data.status || data.task_status || fallback.status),
    ),
    progress: typeof data.progress === "number" ? data.progress : 0,
    result: data.result ?? data.output ?? fallback.result,
    error: formatRemoteTaskError(
      pickStr(data.error || data.fail_reason || data.message || fallback.error),
    ),
    raw: data,
  };
}
