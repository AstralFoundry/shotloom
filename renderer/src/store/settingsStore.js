...
import { migrateCatalogModelsToProviderAdapters } from "@/domain/provider/ProviderAdapterContract";
import {
...
export const settingsStore = reactive({
  storageVersion: 7,
  providerConfigs:
    /** @type {Record<string, import('@/domain/provider/ProviderRegistry').ProviderConfig>} */ ({}),
  protocolAdapters:
    /** @type {import('@/domain/provider/ProviderAdapterContract').ProviderProtocolAdapter[]} */ ([]),
  balance: 0,
  rawQuota: 0,
  usedQuota: 0,
  tokenGroups: [],
...
function normalizeTokenGroups(tokenGroups) {
  return (Array.isArray(tokenGroups) ? tokenGroups : []).map(
    (group, index) => ({
...
function normalizePreferredTextModel(value) {
  ...
  return getModelIdsByType("textGeneration").includes(model)
    ? model
    : "gpt-5.4";
}
function normalizePreferredImageModel(value) {
  const model = typeof value === "string" ? value.trim() : "";
  return getModelIdsByType("imageGeneration").includes(model)
    ? model
    : "gpt-image-2";
}
function normalizePreferredVideoModel(value) {
  const model = typeof value === "string" ? value.trim() : "";
  return getModelIdsByType("videoGeneration").includes(model)
    ? model
    : "grok-imagine-video";
}
...
function toPlainSettings(patch = {}) {
  ...
  const next = {
    ...
    protocolAdapters: [...rawStore.protocolAdapters],
    balance: rawStore.balance,
    rawQuota: rawStore.rawQuota,
    usedQuota: rawStore.usedQuota,
    tokenGroups: normalizeTokenGroups(rawStore.tokenGroups),
    activeTokenGroupId: rawStore.activeTokenGroupId,
    apiKeyValid: rawStore.apiKeyValid,
    accountSyncError: rawStore.accountSyncError,
    projectRootDir: rawStore.projectRootDir,
    modelPollIntervalMs: rawStore.modelPollIntervalMs,
    agentAutoEval: rawStore.agentAutoEval,
    agentAutoLayout: rawStore.agentAutoLayout,
    ...
    canvasActionShortcuts: normalizeCanvasActionShortcuts(
      rawStore.canvasActionShortcuts,
    ),
    layoutAlgorithm:
      rawStore.layoutAlgorithm === "elk-layered"
        ? "elk-layered"
        : "grid-aligned",
...
function applySettings(settings) {
  ...
  const protocolAdapters = clonePlainData(settings.protocolAdapters || []);
  settingsStore.storageVersion = settings.storageVersion;
  settingsStore.providerConfigs = providerConfigs;
  settingsStore.protocolAdapters = protocolAdapters;
  setExternalCatalogModels(
    buildCustomCatalogModels(providerConfigs, protocolAdapters),
  );
  settingsStore.balance = Number.isFinite(settings.balance)
    ? settings.balance
    : 0;
  settingsStore.rawQuota = Number.isFinite(settings.rawQuota)
    ? settings.rawQuota
    : 0;
  settingsStore.usedQuota = Number.isFinite(settings.usedQuota)
    ? settings.usedQuota
    : 0;
  ...
  settingsStore.modelPollIntervalMs = Number.isFinite(
    settings.modelPollIntervalMs,
  )
    ? settings.modelPollIntervalMs
    : 1500;
  ...
  settingsStore.agentPreferredTextModel = normalizePreferredTextModel(
    settings.agentPreferredTextModel,
  );
  settingsStore.agentPreferredImageModel = normalizePreferredImageModel(
    settings.agentPreferredImageModel,
  );
  settingsStore.agentPreferredVideoModel = normalizePreferredVideoModel(
    settings.agentPreferredVideoModel,
  );
  settingsStore.canvasActionShortcuts = normalizeCanvasActionShortcuts(
    settings.canvasActionShortcuts,
  );
  settingsStore.layoutAlgorithm =
    settings.layoutAlgorithm === "elk-layered" ? "elk-layered" : "grid-aligned";
...
export function migrateProviderSettings(settings = {}) {
  let changed = !Array.isArray(settings.protocolAdapters);
  const protocolAdapters = clonePlainData(settings.protocolAdapters || []);
  const adapterById = new Map(
    protocolAdapters.map((adapter) => [adapter.id, adapter]),
  );
  const providerConfigs = Object.fromEntries(
    Object.entries(settings.providerConfigs || {}).map(
      ([providerId, source]) => {
        const config = { ...(source || {}) };
        const legacyModels = Array.isArray(config.models) ? config.models : [];
        const migrated = migrateCatalogModelsToProviderAdapters(legacyModels);
        const localAdapters = [
          ...(Array.isArray(config.protocolAdapters)
            ? config.protocolAdapters
            : []),
          ...migrated.adapters,
        ];
        const remappedAdapterIds = new Map();
        for (const adapter of localAdapters) {
          if (!adapter?.id) continue;
          let targetId = adapter.id;
          const existing = adapterById.get(targetId);
          if (
            existing &&
            JSON.stringify(existing) !== JSON.stringify(adapter)
          ) {
            const baseId = `${providerId}-${adapter.id}`;
            targetId = baseId;
            let suffix = 2;
            while (adapterById.has(targetId))
              targetId = `${baseId}-${suffix++}`;
          }
          remappedAdapterIds.set(adapter.id, targetId);
          if (!adapterById.has(targetId)) {
            const normalized = { ...clonePlainData(adapter), id: targetId };
            protocolAdapters.push(normalized);
            adapterById.set(targetId, normalized);
          }
        }
        const currentBindings = Array.isArray(config.modelBindings)
          ? config.modelBindings
          : [];
        const bindingIds = new Set(
          currentBindings.map((binding) => binding.id),
        );
        config.modelBindings = [
          ...currentBindings,
          ...migrated.bindings.filter((binding) => !bindingIds.has(binding.id)),
        ].map((binding) => ({
          ...binding,
          adapterId:
            remappedAdapterIds.get(binding.adapterId) || binding.adapterId,
        }));
        if (
          Object.hasOwn(config, "models") ||
          Object.hasOwn(config, "protocolAdapters")
        )
          changed = true;
        delete config.models;
        delete config.protocolAdapters;
  ...
  return {
    changed,
    settings: { ...settings, protocolAdapters, providerConfigs },
  };
}
...
export async function loadAppSettings() {
  ...
  try {
    ...
    if (migrated.changed)
      settings = await desktopApi.settings.set(migrated.settings);
    if (desktopApi.platform !== "browser") {
      ...
      if (ensuredRoot && ensuredRoot !== requestedRoot) {
        settings = await desktopApi.settings.set({
          ...settings,
          projectRootDir: ensuredRoot,
        });
      }
    }
