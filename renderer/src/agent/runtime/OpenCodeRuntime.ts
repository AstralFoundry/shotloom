...
import { availableAgentSkills, type StoredSkill } from '../tools/catalogTools';
...
import { activeProductionPlan, type ProductionPlan } from './productionPlanStore';
import { parseSkillRoutingDecision, type SkillRoutingDecision } from './skillRoutingDecision';
...
function agentProfiles(skills: StoredSkill[]) {
  ...
  return {
    ...
    'intent-router': {
      mode: 'subagent', maxSteps: 30,
      description: 'Resolve source maturity, requested deliverable, production scope and capability conflicts',
      prompt: `Choose the single best Skill from the provided catalog by understanding the complete user request, including negation and requested deliverable. Never route by isolated keywords. Also decide whether the request continues the supplied active Production Plan. Return only the requested JSON object. Do not use tools.\n\nAvailable Skill metadata:\n${JSON.stringify(skillCatalog)}\n\n${shared('intent-router')}`,
      tools: analysisTools,
    },
    'production-planner': {
...
function configureModel(model: string, skills: StoredSkill[], workspaceDirectory: string) {
...
function systemPrompt(skill: any) {
  ...
  const contracts = formatContractsBlock(contractsForAgentType('shotloom', skill?.contracts));
  ...
  return [
    ...
    'For a non-trivial production request, inspect runtime capabilities first. The Router decides production scope from the full user message. If the scope is genuinely unresolved, use request_clarification and formulate the question and options for the actual context. Do not ask again when the user has already stated the scope clearly. Never infer execution merely because the user pasted a complete script.',
    ...
    'Use intent-router, production-planner, stage-executor, or result-reviewer only for their named bounded responsibility. The primary agent owns user interaction and stage lifecycle.',
...
async function ensureMcpConnected(client: OpenCodeClient, directory: string, refresh = false) {
  let statuses = responseData<any>(await client.mcp.status({ query: { directory } }));
  if (statuses?.shotloom?.status === 'connected' && !refresh) return;
...
async function routeSkill(
  ...
  signal?: AbortSignal,
): Promise<SkillRoutingDecision<StoredSkill>> {
  const session = responseData<any>(await client.session.create({
    body: { parentID: parentSessionId, title: 'Skill Router' },
    query: { directory },
  }));
  if (!session?.id) throw new Error('OpenCode 没有为 Skill Router 返回 Session ID');
  ...
  const routingPrompt = [
    'Select one Skill by understanding the full request. Account for negation and the requested deliverable; never select from isolated keyword matches.',
    'Also decide whether this request continues the active Production Plan. A new or unrelated request must return false.',
    'Return exactly one JSON object: {"skillId":"...","continueProductionPlan":true|false,"reason":"short explanation"}.',
    `Available Skills: ${JSON.stringify(catalog)}`,
    `Active Production Plan: ${JSON.stringify(activePlan)}`,
  ...
  try {
    const response = responseData<any>(await client.session.prompt({
      ...
      body: {
        ...
        system: 'Return only the requested JSON object. Do not call tools and do not answer the user.',
        parts: await promptParts(routingPrompt, attachments),
      },
      signal,
    }));
    const error = runtimeErrorText(response?.info?.error || response?.error);
    if (error) throw new Error(error);
    return parseSkillRoutingDecision(textFromParts(response?.parts || []), skills);
  } finally {
    await client.session.delete({ path: { id: session.id }, query: { directory } }).catch(() => undefined);
  }
}
...
export async function runOpenCodeAgent(
  ...
  const toolContext: AgentToolContext = {
    requestId, turnId: `${requestId}:root`, projectKey, conversationId,
  ...
  try {
    const configured = configureModel(model, skills, directory);
    ...
    const routing = await routeSkill(
      client,
      sessionId,
      directory,
      model,
    ...
    skill = routing.skill;
    toolContext.loadedSkillIds.add(skill.id);
    state.set('skillRoutingDecision', {
      skillId: skill.id,
      continueProductionPlan: routing.continueProductionPlan,
      reason: routing.reason,
      usedFallback: routing.usedFallback,
    });
    if (routing.continueProductionPlan && existingPlan) {
      state.set('activeProductionPlanId', existingPlan.id);
      state.set('activeProductionPlan', existingPlan);
    }
    await activateOpenCodeToolBridge({ context: toolContext, model });
    await ensureMcpConnected(client, directory, true);
  } catch (cause) {
    controller.abort();
  ...
  try {
    const routingDecision = state.get('skillRoutingDecision') as JsonObject | undefined;
    const boundPlan = state.get('activeProductionPlan') as ProductionPlan | undefined;
    const contextSuffix = [
      ...
      routingDecision ? `Router decision: ${JSON.stringify(routingDecision)}` : '',
      boundPlan ? `Active Production Plan selected by Router: ${JSON.stringify(boundPlan)}` : '',
    ].filter(Boolean).join('\n');
    ...
    emit({ type: 'run_status', status: aborted ? 'cancelled' : 'failed', error: cause instanceof Error ? cause.message : String(cause), createdAt: new Date().toISOString() });
    throw cause;
  } finally {
    controller.abort();
