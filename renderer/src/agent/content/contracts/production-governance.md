...
- Planner 决定制作阶段、工作项、依赖、节点分组和呈现顺序；Executor 使用细粒度画布工具逐项落实；Reviewer 只根据局部画布事实、任务终态和成功回执核验。布局代码只处理真实拓扑、明确节点集合、尺寸和碰撞，不替三者从标题、类型或关键词推断制作语义。
...
- 画布写入发生 revision 冲突时同样重新读取相关局部事实，只重做尚未成功的动作；创建、连接或启动回执已经成功的对象不得重复创建或重复启动。
