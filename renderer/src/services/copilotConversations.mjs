...
export function ensureCopilotConversations(project) {
  ...
  const original = Array.isArray(project.copilotConversations)
    ? project.copilotConversations
    : [];
  let conversations = original.filter(
  ...
  for (const conversation of conversations) {
    ...
    else if (typeof conversation.id !== "string")
  ...
  if (
    !conversations.some(
      (item) => item.id === project.activeCopilotConversationId,
    )
  ) {
...
export function switchCopilotConversation(project, conversationId) {
  ensureCopilotConversations(project);
  if (!project.copilotConversations.some((item) => item.id === conversationId))
    return null;
...
export function deleteCopilotConversation(project, conversationId) {
  ...
  const index = project.copilotConversations.findIndex(
    (item) => item.id === id,
  );
  ...
  if (Array.isArray(project.agentSteps)) {
    project.agentSteps = project.agentSteps.filter(
      (step) => String(step?.conversationId || "") !== id,
    );
  }
  ...
  if (deletingActive) {
    ...
    project.activeCopilotConversationId =
      project.copilotConversations[nextIndex].id;
  }
  return getActiveCopilotConversation(project);
}
