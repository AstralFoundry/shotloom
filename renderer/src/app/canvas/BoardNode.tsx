...
export const BoardNode: WorkflowNodeRenderer = memo(
    ...
    const boardRef = useRef(
      ensureBoardData({
        boardData: node.boardData
          ? structuredClone(toRaw(node.boardData))
          : undefined,
      }),
    );
    useEffect(() => {
      ...
      >
        ...
        <div className="board-head">
          ...
          <button
            title="导出图片"
            onClick={() =>
              actions.exportBoard(node.id, boardToDataUrl(boardRef.current))}
          >
            <IconSymbol name="download" />
          </button>
          <button
            title="添加图片"
            onClick={() => actions.addBoardImage(node.id)}
          >
