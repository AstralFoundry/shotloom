...
test("命名空间工具预先暴露，但执行前必须加载对应 Skill", () => {
  ...
  const context = {
    ...
    loadedSkillIds: new Set(["video-production"]),
    attachments: [],
  ...
  assert.deepEqual(
    listAgentTools(context).map((tool) => tool.id),
...
test("默认 Agent 工具以真实注册表为准并允许 Runtime 重复激活", () => {
  const source = readFileSync(
    new URL(
      "../renderer/src/agent/tools/registerDefaultTools.ts",
      import.meta.url,
    ),
    "utf8",
  );
  assert.match(source, /hasAgentTool\(["']get_canvas["']\)/);
  assert.match(source, /hasAgentTool\(["']inspect_provider_setup["']\)/);
