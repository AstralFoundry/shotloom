...
const tools = readFileSync(
  new URL(
    "../renderer/src/agent/tools/productionPlanTools.ts",
    import.meta.url,
  ),
  "utf8",
);
...
const planStore = readFileSync(
  new URL(
    "../renderer/src/agent/runtime/productionPlanStore.ts",
    import.meta.url,
  ),
  "utf8",
);
const lifecycle = readFileSync(
  new URL("../renderer/src/agent/tools/lifecycleTools.ts", import.meta.url),
  "utf8",
);
const catalog = readFileSync(
  new URL("../renderer/src/agent/tools/catalogTools.ts", import.meta.url),
  "utf8",
);
const governance = readFileSync(
  new URL(
    "../renderer/src/agent/content/contracts/production-governance.md",
    import.meta.url,
  ),
  "utf8",
);
...
test("Production Plan 提供持久化阶段工具而不是临时文本计划", () => {
  ...
  assert.match(
    store,
    /productionPlans: Array\.isArray\(project\?\.productionPlans\)[\s\S]*schemaVersion === 2/,
  );
});
...
test("复杂制作是否建立计划由主 Agent 决定，工具只校验客观执行状态", () => {
  assert.doesNotMatch(
    runtime,
    /requiresProductionPlan|productionPlanPreparedForRun/,
  );
  assert.doesNotMatch(canvas, /首次修改画布前必须先用 plan_write/);
  assert.doesNotMatch(
    canvas,
    /activeProductionPlanId|executionMode !== 'execute'|已通过执行计划审核/,
  );
});
...
test("复杂制作范围由主 Agent 判断并按上下文提问", () => {
  ...
  assert.doesNotMatch(
    lifecycle,
    /PRODUCTION_SCOPE_QUESTION_ID|productionScopeOptions|PLAN_CANVAS_OPTION/,
  );
  ...
  assert.doesNotMatch(
    catalog,
    /isAvailable: \(\{ state \}\).*runtimeCapabilitiesInspected/,
  );
});
...
test("Production Plan v2 用逐项 runtime ref 核验真实节点和任务", () => {
  ...
  assert.doesNotMatch(
    planStore,
    /plan_review|result_review|approve_plan|approve_result|前置制作阶段尚未完成/,
  );
