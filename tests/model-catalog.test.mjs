...
const providerCatalog = JSON.parse(
  readFileSync(
    new URL("../renderer/src/config/provider-catalog.json", import.meta.url),
    "utf8",
  ),
);
const agentContract = JSON.parse(
  readFileSync(
    new URL(
      "../renderer/src/config/agent-action-contract.json",
      import.meta.url,
    ),
    "utf8",
  ),
...
function bindEndpoint(endpoint, modelId) {
...
function compileBuiltInCatalog(source) {
  const adapters = source.entries.filter((entry) => entry.kind === "adapter");
  const models = source.entries
    .filter((entry) => entry.kind === "model")
    .map((binding) => {
      const adapter = adapters.find(
        (entry) =>
          entry.provider === binding.provider && entry.id === binding.adapterId,
      );
      assert.ok(
        adapter,
        `${binding.provider}/${binding.id} 引用了不存在的 Adapter ${binding.adapterId}`,
      );
      const modes = structuredClone(adapter.modes).map((mode) => ({
        ...mode,
        endpoint: bindEndpoint(mode.endpoint, binding.id),
        taskEndpoint: bindEndpoint(mode.taskEndpoint, binding.id),
        resultEndpoint: bindEndpoint(mode.resultEndpoint, binding.id),
      }));
      return {
        id: binding.id,
        requestModelId: binding.id,
        name: binding.name,
        provider: binding.provider,
        type: binding.type,
        sortOrder: binding.sortOrder,
        enabled: binding.enabled !== false,
        defaultMode: binding.defaultMode || adapter.defaultMode,
        modes,
      };
    });
...
const catalog = compileBuiltInCatalog(providerCatalog);

test("内置厂商使用与自定义厂商相同的 Adapter / Binding 协议", () => {
  assert.equal(providerCatalog.schema, "shotloom.provider-catalog");
  assert.equal(providerCatalog.providerDefinitionVersion, 1);
  assert.equal(Object.hasOwn(providerCatalog, "models"), false);
  const adapters = providerCatalog.entries.filter(
    (entry) => entry.kind === "adapter",
  );
  const bindings = providerCatalog.entries.filter(
    (entry) => entry.kind === "model",
  );
  assert.ok(adapters.length > 0);
  assert.ok(bindings.length > 0);
  assert.ok(
    adapters.length < bindings.length,
    "至少应有多个模型复用同一个 Adapter",
  );
  for (const adapter of adapters) {
    assert.ok(adapter.provider);
    assert.ok(adapter.id);
    assert.ok(adapter.name);
    assert.ok(adapter.type);
    assert.ok(Array.isArray(adapter.modes) && adapter.modes.length > 0);
  }
  for (const binding of bindings) {
    assert.ok(binding.provider);
    assert.ok(binding.id);
    assert.ok(binding.name);
    assert.ok(binding.type);
    assert.ok(binding.adapterId);
  }
  ...
  for (const model of catalog.models) {
    ...
    for (const mode of model.modes) {
      ...
      assert.ok(
        mode.requestTemplate && typeof mode.requestTemplate === "object",
      );
      ...
      if (mode.isAsync)
        assert.ok(mode.taskEndpoint?.path?.includes("{taskId}"));
      for (const param of mode.params) {
        assert.ok(param.presentation && typeof param.presentation === "object");
        assert.ok(param.presentation.control);
        if (!Array.isArray(param.options) || !param.options.length) continue;
        assert.ok(
          param.options.includes(param.default),
          `${model.id}/${mode.id}/${param.key} 默认值不在 options 中`,
        );
      }
    }
  }
...
test("GPT 5.6 的工具请求显式关闭 Chat Completions 推理", () => {
  ...
  for (const mode of model.modes) {
    ...
    assert.equal(
      renderProtocolTemplate(mode.requestTemplate, {
        model: model.id,
...
test("DeepSeek V4 Pro 为 Agent 保留 32K 输出与推理预算", () => {
  const model = catalog.models.find((item) => item.id === "deepseek-v4-pro");
  assert.ok(model);
  for (const mode of model.modes) {
    assert.equal(mode.outputConstraints.maxTokens, 32768);
    assert.ok(
      mode.params
        .find((param) => param.key === "maxTokens")
        .options.includes(32768),
    );
  }
});
...
test("GPT Image 2 用真实 size 参数提供统一比例选项", () => {
  ...
  for (const mode of model.modes) {
    ...
    assert.equal(size.presentation.control, "ratio");
...
test("OpenAI GPT 画布文本请求使用 max_completion_tokens", () => {
  const models = catalog.models.filter(
    (item) => item.provider === "openai" && item.type === "textGeneration",
  );
  assert.ok(models.length > 0);
  for (const model of models) {
    for (const mode of model.modes) {
      assert.equal(
        mode.requestTemplate.max_completion_tokens,
        "{{params.maxTokens}}",
      );
...
test("MiniMax H3 使用官方 Video Generation V2 任务协议", () => {
  ...
  assert.deepEqual(
    model.modes.map((mode) => mode.id),
    ["text-to-video", "first-frame-to-video"],
  );
  for (const mode of model.modes) {
    assert.deepEqual(mode.endpoint, {
      method: "POST",
      path: "/v2/video_generation",
      scope: "root",
    });
    assert.deepEqual(mode.taskEndpoint, {
      method: "GET",
    ...
    assert.deepEqual(
      mode.params.find((param) => param.key === "resolution").options,
      ["768P", "2K"],
    );
    assert.deepEqual(
      mode.outputConstraints.durations,
  ...
  assert.deepEqual(
    textMode.params.find((param) => param.key === "aspectRatio").options,
    ["21:9", "16:9", "4:3", "1:1", "3:4", "9:16"],
  );

  const imageMode = model.modes.find(
    (mode) => mode.id === "first-frame-to-video",
  );
...
test("Kling 3.0 系列使用官方 API 2.0 视频任务协议", () => {
  ...
  assert.deepEqual(
    turbo.modes.map((mode) => mode.endpoint.path),
    ["/text-to-video/kling-3.0-turbo", "/image-to-video/kling-3.0-turbo"],
  );
  assert.deepEqual(
    standard.modes.map((mode) => mode.endpoint.path),
    ["/text-to-video/kling-3.0", "/image-to-video/kling-3.0"],
  );
  ...
  assert.equal(
    turboImage.requestFields.imageContentFormat,
    "kling-first-frame",
  );
  assert.deepEqual(
    renderProtocolTemplate(turboImage.requestTemplate, {
      klingContents: [
        { type: "prompt", text: "让人物转头" },
        { type: "first_frame", url: "https://example.com/first.png" },
      ],
      params: { resolution: "1080p", duration: 5, watermark: false },
    }).contents,
    [
      { type: "prompt", text: "让人物转头" },
      { type: "first_frame", url: "https://example.com/first.png" },
  ...
  assert.deepEqual(
    renderProtocolTemplate(omniMode.requestTemplate, {
      klingContents: [
...
test("Seedance 2.0 Fast/Mini 按官方规格限制为 480p/720p", () => {
  for (const id of [
    ...
    const mode = model.modes.find(
      (item) => item.id === "text-or-reference-image-to-video",
    );
    assert.deepEqual(mode.inputConstraints.images, {
      min: 0,
      max: 9,
      roles: ["referenceImage"],
    });
...
test("Seedance 2.0 使用方舟官方视频任务协议", () => {
  const model = catalog.models.find(
    (item) => item.id === "doubao-seedance-2-0-260128",
  );
  assert.equal(model.provider, "bytedance");
  assert.deepEqual(
    model.modes.map((mode) => mode.id),
    ["text-to-video", "omni-reference-to-video"],
  );
  for (const mode of model.modes) {
    assert.deepEqual(mode.endpoint, {
      method: "POST",
  ...
  const referenceMode = model.modes.find(
    (mode) => mode.id === "omni-reference-to-video",
  );
  assert.deepEqual(
    referenceMode.inputVariants.map((variant) => variant.inputMode),
    ["firstLastFrame"],
  );
  assert.deepEqual(referenceMode.inputConstraints.images, {
    min: 0,
    max: 9,
    roles: ["referenceImage"],
  });
...
test("Grok Imagine Video 和全部 Veo 3.1 模型实际编译单图生视频", () => {
  ...
  const grokMode = grok.modes.find(
    (mode) => mode.id === "text-or-image-to-video",
  );
  assert.deepEqual(grokMode.inputConstraints.images, {
    min: 0,
    max: 1,
    roles: ["referenceImage"],
  });
  ...
  assert.equal(
    Object.hasOwn(
      renderProtocolTemplate(grokMode.requestTemplate, {
        model: grok.id,
        prompt: "纯文本",
        params: { duration: 8, aspectRatio: "16:9", resolution: "720p" },
      }),
      "image",
    ),
    false,
  ...
  assert.deepEqual(
    renderProtocolTemplate(grokMode.requestTemplate, {
      model: grok.id,
      prompt: "图生视频",
      imageObject: { url: "https://example.com/input.png" },
      params: { duration: 8, aspectRatio: "16:9", resolution: "720p" },
    }).image,
  ...
  for (const id of [
    ...
    const model = catalog.models.find((item) => item.id === id);
    const mode = model.modes.find(
      (item) => item.id === "text-or-image-to-video",
    );
    assert.deepEqual(mode.inputConstraints.images, {
      min: 0,
      max: 1,
    ...
    assert.deepEqual(
      renderProtocolTemplate(mode.requestTemplate, {
        prompt: "纯文本",
        params: { duration: 8, aspectRatio: "16:9" },
      }).instances,
      [{ prompt: "纯文本" }],
    );
    assert.deepEqual(
...
test("内置模型只使用唯一声明式执行协议", () => {
  ...
  for (const legacyField of [
    ...
    assert.equal(
      serialized.includes(`"${legacyField}"`),
      false,
      `仍包含旧执行字段 ${legacyField}`,
    );
  }
  for (const model of catalog.models) {
    for (const mode of model.modes) {
      assert.ok(
        mode.requestTemplate,
        `${model.id}/${mode.id} 缺少 requestTemplate`,
      );
      assert.ok(
        mode.resultTextPath ||
          mode.resultUrlPath ||
...
test("媒体角色与首尾帧业务槽位保持分离", () => {
  ...
  assert.ok(
    agentContract.commonProperties.inputMode.enum.includes("firstLastFrame"),
  );
...
test("每个可接收素材的视频供应商 mode 显式声明画布输入语义", () => {
  for (const model of catalog.models.filter(
    (item) => item.type === "videoGeneration",
  )) {
    for (const mode of model.modes) {
      const input = mode.inputConstraints || {};
      const hasMedia =
        (input.images?.max || 0) > 0 ||
        (input.videos?.max || 0) > 0 ||
        (input.audios?.max || 0) > 0;
      if (!hasMedia) continue;
      assert.ok(mode.inputMode, `${model.id}/${mode.id} 缺少 inputMode`);
      assert.ok(
        Array.isArray(mode.inputSlots) && mode.inputSlots.length,
        `${model.id}/${mode.id} 缺少 inputSlots`,
      );
    }
  }
});
...
test("Agent 生成节点契约只使用顶层 prompt/model", () => {
  ...
  assert.match(
    agentContract.commonProperties.config.description,
    /不得包含 prompt 或 model/,
  );
});
